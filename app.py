...
with open('Models/Pickle_SVMC_Model.pkl', 'rb') as f:
...
def my_form_post():
    if request.method == 'POST':
        ...
        if target==1:
            health_status = 'You have a heart disease'
        else:
            health_status = 'You are healthy'

        return render_template('home.html', target = target, sale_making = health_status)
    else:
        return render_template('home.html')
